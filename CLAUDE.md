...
- **01~07시, 매시가 목표지만 실제로는 하루 2번만 돈다 — "슬롯을 스킵"하는 게
  아니라 "지연되면서 그 사이 트리거가 버려지는" 것이다(2026-09-26, 실행 기록
  26건 전수 확인으로 정정 — 이전엔 "부하로 스킵"이라고만 적어뒀었다).**
  `.github/workflows/self-improve-autoplay.yml`의 cron은 `7 16-22 * * *`(UTC) =
  KST 01:07~07:07 매시 7개 슬롯을 목표로 `bot/autoPlay.ts`를 돌려 AI가 직접
  몇 판을 플레이하고 피드백 이슈를 쌓는다. 실제 패턴: 그날 첫 실행이 목표보다
  **2.5~4시간 늦게** 뜨고(예: 01:07 목표 → 04:37~04:57 실행), 그 사이 지나간
  매시 트리거(02·03·04시 등)는 대기열에 쌓이거나 나중에 몰아서 도는 게 아니라
  그냥 사라진다 — 그래서 하루 7번이 아니라 "이른 실행 1번 + 늦은 실행 1번"
  딱 2번만 돈다. 지연 폭이 날마다 달라서(2.5~4시간을 오간다) 몇 시에 도는지도
  예측이 안 된다. GitHub 공식 문서는 "부하가 높으면 예약 실행이 지연·누락될
  수 있다"고만 하고, 이 레포가 왜 이렇게까지 밀리는지는 GitHub 내부 사정이라
  우리 쪽에서 원인을 더 파고들 방법이 없다 — 그래서 **"몇 시에 도는지"에
  의존하던 로직(제미나이 추측자 시간대 필터)을 없애고 "판 수"로 보정했다**
  (2026-09-26 — `AUTO_PLAY_GAMES` 기본값을 2→6으로, 제미나이는 시간대 대신
  "그 실행의 첫 판"으로만 판단하게 `autoPlay.ts` 수정. 자세한 근거는 그
  워크플로 파일의 cron 주석과 `autoPlay.ts`의 `GUESSER_OVERRIDE` 주석).
...
- **08시, 하루 한 번(설계상 목표) — `.github/workflows/self-improve.yml`의
  ...
  마지막 실행(07시)과 Groq 호출이 겹치는 걸 피하려고 한 시간 늦췄다. 위와 같은
...
- **제미나이 무료 티어 quota는 하루 20 RPD.** `propose.mjs`와 `autoPlay.ts`의 추측자
  ...
  된다. quota 에러가 나면 `propose.mjs`가 전체 에러 본문을 로그에 찍으니 RPD/RPM부터
