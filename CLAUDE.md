...
- **DB 없음, 세션은 암호화 토큰으로.** 라운드 사이 상태(제시어)를 서버가 안
  들고 있고, 클라이언트가 AES-256-GCM으로 암호화된 토큰을 들고 다닌다
  (`apps/backend/src/routes/gameToken.ts`). 서버가 매 요청마다 새로 뜰 수
  있다는 전제(서버리스 배포)와 잘 맞는다. DB로 옮기게 되면 `SessionPayload`를
  테이블 row로 옮기고 이 파일 구현만 바꾸면 된다 — 호출부는 그대로 쓴다.
...
- **GitHub Actions는 존재하지 않는 시크릿을 참조해도 그 env를 빈 문자열로
  채운다(undefined가 아니라).** `propose.mjs`의 `SELFIMPROVE_BOT_*` →
  `BOT_*` fallback 체인이 `??`가 아니라 `||`를 쓰는 이유가 이거다 — `??`는
  `""`를 "값 있음"으로 쳐버려서 진짜 값이 있는 `BOT_*`로 안 넘어간다
  (2026-09-16 실제로 이 버그로 하루 날렸다).
...
- 단어를 DB(Supabase)로 옮길지 — 지금은 결정 안 했고, `wordPool.ts`의
  로더만 갈아끼우면 되게 분리해뒀다.
- 라운드당 묘사 개수(5개 고정 vs 밸런스 보고 4개) — `docs/게임기획서_v2.md`
  §8 참고.
