...
- **`judgeGuess()`의 loose 판정(포함 관계면 정답 처리, `wordGuessBot.ts`)엔 예외가
  셋 있다(2026-09-26).** 예전엔 부분 문자열 포함이면 무조건 정답이라, 풀 전수 검사
  결과 다른 제시어를 댄 추측이 정답 처리되는 쌍이 한국어 89·영어 3개였다("배"→배드민턴,
  "고래"→돌고래, "ant"→elephant — "줄자"에 "자"도 같은 문제). 지금은 ① 추측이 풀의
  다른 제시어나 그 동의어면 오답(`wordPool.ts`의 `allPoolTerms()`), ② 한국어는 짧은
  쪽이 두 글자 이상일 때만 포함 인정, ③ 영어는 철자가 아니라 단어 단위로 포함을 본다
  (복수형 s·관사 a/an/the는 무시). 제시어를 추가할 땐 한 글자 단어나 다른 제시어를
  품은 합성어가 들어가도 판정은 안전하지만, accept에 다른 제시어와 겹치는 동의어를
  넣으면 안 된다.
