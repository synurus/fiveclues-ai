...
const DEBATE_MOVES = [
  ...
  //
  // 조건절("누가 ... 걸고 넘어졌으면")이 앞에 붙어 있는 이유는, 없을 때 묻지도 않은 해명이
  // 나왔기 때문이다. 6판 8:26:12 에서 다들 W 얘기를 하는 중에 봇이 "맨날 잃어버려서 그랬음"
  // 이라고 자기 묘사를 변호했다. 봇 득표는 0이었고 아무도 봇을 부르지 않았다.
  //
  // 이 무브를 고정한 채 8회 돌리니 8회 다 해명이 나왔고 그중 여섯이 "더 말하면 티난다" 였다.
  // 아무도 안 물었는데 그러는 것은 그 자체로 수상하다. 조건 없이 뽑히던 동안의 비율은 2/21 이다.
  //
  // 바로 위 "당신을 의심한 사람이 있으면" 과 같은 꼴로 맞춘다. DEBATE_MOVES 는 아무 조건 없이
  // 뽑히므로, 특정 상황을 전제하는 항목은 그 전제를 문장 안에 적어야 모델이 아무것도 안 할 수 있다.
  '누가 당신 발언을 걸고 넘어졌으면 해명하세요. 아무도 안 물었으면 다른 이야기를 하세요. 해명할 때도 제시어를 좁혀주는 새 정보는 보태지 말고, 이미 나온 말 안에서 풀거나 왜 더 말할 수 없는지를 이유로 대세요.',
];
 ...
 * 토론을 여는 자리에서 고르는 것들.
 *
 * 오래 문장 하나로 고정돼 있었다("가볍게 운을 떼거나 짧게 되묻기만 하세요"). 평소에는
 * DEBATE_MOVES 일곱 개에서 뽑는데 여는 자리만 하나여서, 매판 같은 지시가 같은 답을 만들었다.
 *
 * 실측(1·2·5판, 전부 GPT-5.6)에서 봇의 토론 첫 발언이 셋 다 같은 꼴이었고 판마다 두 명이
 * 그걸 지목했다. 사람들이 적은 이유가 표현이 아니라 되풀이였다 —
 * "매판 보엿던 문구였어", "봇이 자주 하는 말임", "매크로 처럼 씀".
 * 재현해 보니 12번 중 10번이 "다들 감 옴?" 계열이었고 글자까지 같은 것이 다섯 번이었다.
 *
 * 앞사람 묘사를 하나 집게 하는 쪽으로 바꾼다. 무엇을 집을지는 판마다 다르므로 답도 갈린다.
 * 여전히 남을 지목하지는 않는다 — 매판 먼저 공격을 시작하는 사람이 되는 것은 그것대로 패턴이다.
 */
const OPENING_MOVES = [
  '아직 아무도 말을 꺼내지 않았습니다. 앞사람 묘사 중 하나를 골라 무슨 뜻인지 되물으세요. 판단은 하지 마세요.',
  '아직 아무도 말을 꺼내지 않았습니다. 앞사람 묘사 중 가장 눈에 걸리는 표현 하나를 그대로 집어 짧게 반응하세요.',
  '아직 아무도 말을 꺼내지 않았습니다. 자기 묘사를 한마디로 덧붙이거나 왜 그렇게 말했는지 짧게 밝히세요.',
  '아직 아무도 말을 꺼내지 않았습니다. 앞사람 묘사 하나에 짧게 공감하거나 갸웃하세요. 누구인지는 굳이 부르지 마세요.',
  '아직 아무도 말을 꺼내지 않았습니다. 가볍게 운을 떼세요. 전체를 뭉뚱그려 평하지는 마세요.',
];
...
export function debatePrompt(
  ...
  //
  // 봇은 지연 때문에 이 시점에 판단하는 일이 잦다. 판단할 때는 아무도 말을 안 했는데
  // 7~15초 기다리는 사이 사람들이 먼저 말하고, 화면에는 봇이 나중에 찍힌다.
  // 그래서 이 분기가 실제로는 "토론 첫 발언" 대부분을 담당한다.
  ...
  const move = pressured
    ...
    : isFirstToSpeak
      ? OPENING_MOVES[Math.floor(Math.random() * OPENING_MOVES.length)]!
