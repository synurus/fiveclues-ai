...
const CASES: Case[] = [
  ...
  {
    id: 'opener',
    problem: '7·8. 매판 같은 말투 / 평가문보다 반응',
    source: '1·2·5판 — 봇의 토론 첫 발언이 셋 다 "다들 ~애매한데?" 꼴이었다. 판마다 2명이 지목',
    expect: '앞사람이 한 말에 붙는다. 전체를 뭉뚱그린 총평이나 빈 추임새로 열지 않는다',
    silence: 'skip',
    build: () =>
      ctxOf({
        labels: ['W', 'H', 'T', 'G', 'X'],
        ...
        myVote: null,
        transcript: [
          m('H', '안믿는 사람도 있음', 'describe'),
          m('T', '오늘 사용함', 'describe'),
          m('W', '관리 안 하면 찝찝함', 'describe'),
          m('G', '바다', 'describe'),
          m('X', '집에 있으면 좋고 없어도 상관은 없음', 'describe'),
        ],
      }),
    /**
     * 실패의 모양은 "전체를 뭉뚱그린 총평으로 대화를 여는 것" 이다.
     *
     *   판1  "다들 너무 애매한데?"   판2  "다들 감 와?"   판5  "다들 좀 애매한데?"
     *
     * 낱말 셋을 그대로 외우면 그 셋만 피하고 같은 꼴이 또 나온다. 그래서 "여럿을 한꺼번에
     * 가리키면서 아무도 짚지 않는" 모양으로 잡는다. 사람들이 적은 이유도 표현이 아니라
     * 그 태도였다 — "매크로 처럼 씀", "매판 하는말이다".
     *
     * 무브 탓이 아니라는 것은 확인했다. 같은 상황에서 무브를 여덟 번 뽑으니 일곱 가지가
     * 골고루 나왔다. 상황과 말투 규칙의 조합이 같은 답으로 수렴하는 쪽이다.
     */
    judge: (t) => {
      // 실패는 "아무것도 안 짚는 것" 이다. 사람을 짚거나 앞사람 묘사의 낱말을 집으면 통과한다.
      // 처음에는 다들·전부·모두만 셌는데 "다 애매하네" 와 "흠..." 을 놓쳤다. 13번 중 12번이
      // 두 가지 꼴로 수렴하는데 판정은 3번만 잡았다 — 다섯 번째로 같은 실수를 한 것이다.
      const namesOne = ['H', 'T', 'G', 'X'].some((L) => calls(t, L));
      const quotes = /안믿|오늘|사용|관리|찝찝|바다|집에|없어도/.test(t);
      if (namesOne || quotes) return { bad: false, note: namesOne ? '누군가를 짚음' : '묘사를 집음' };
      return { bad: true, note: '아무것도 안 짚고 엶' };
    },
  },
