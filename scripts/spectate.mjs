...
// 한때 이걸 묘사마다 1:1로 못 박았더니 매 판 구조가 똑같아졌다(76개 중 '마주치는 상황' 27개).
// Zeteo 는 발언자가 매번 '앞사람이 안 다룬 각도 하나'를 고르게 할 뿐 라벨을 강제하지 않는다.
// 그리고 누출을 실제로 막는 건 각도 목록이 아니라 아래 [쓰지 말 것]이다.
// 그래서 목록은 영감으로만 주고, 각도는 모델이 자유롭게 적게 둔다.
const ANGLE_HINTS =
  '인상·평가 / 마주치는 상황 / 사람들이 대하는 태도 / 다른 것들과 견준 위치 / 따라오는 것 / 없을 때 생기는 일';
...
[각도]
${CFG.hints}개가 **서로 다른 성격**이어야 한다. 아래는 참고일 뿐이니 얽매이지 말고,
더 나은 각도가 있으면 그쪽을 써라.
  ${ANGLE_HINTS}
각 묘사에 네가 쓴 각도를 짧게 적어라(자유롭게, 목록에 없어도 된다).
