...
function buildSystemPrompt() {
  return (
    ...
    `7. 이 파일은 이미 여러 번 자동 수정됐을 수 있다. "쓰지 말 것" 목록에 새 금지를 얹기 전에, ` +
    `먼저 그 증상이 "이미 있는 금지 규칙이 너무 넓어서" 생긴 건 아닌지부터 확인해라 — ` +
    `피드백이 "너무 추상적이다/애매하다/뭔지 모르겠다"류라면 대개 기존 규칙이 정보가 될 만한 ` +
    `묘사까지 막은 것이니, 이럴 땐 새 금지를 추가하지 말고 기존 규칙의 범위를 좁혀라 ` +
